...
#   5. Données de démonstration « Atelier Lumière »   (--seed)
...
for arg in "$@"; do
  case "$arg" in
    ...
    -h|--help)
      ...
      printf '  --seed       Créer les données de démonstration « Atelier Lumière » (dev)\n'
...
if [ "$DO_SEED" = 1 ]; then
  ...
  if [ "$IS_PROD" = 1 ]; then
    ...
    bun run db:seed:demo && \
      ok "organisation « Atelier Lumière » créée (owner/member/viewer @atelier-lumiere.local, mot de passe demo-password)" || \
...
if [ "$IS_PROD" = 1 ]; then
  ...
  printf '    3. Déploie via %sinfra/prod/%s (voir %sdocs/operations/%s).\n' "$BOLD" "$RESET" "$DIM" "$RESET"
