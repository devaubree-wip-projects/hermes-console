...
function isCriticalRoute(path: string) {
  return ["/api/auth/", "/agents", "/installations", "/api/runtime/enroll"].some((part) =>
    path.includes(part),
  );
}
...
const inventoryMarkdown = `# Architecture inventory\n\n` +
  `Generated from commit \`${commit}\` with \`bun run audit:architecture\`. Secrets are never read; environment inventory uses tracked references only.\n\n` +
  `## Public API endpoints\n\n| Methods | Path | Route | Critical |\n|---|---|---|---|\n` +
  endpoints.map((endpoint) => `| ${endpoint.methods.join(", ")} | \`${endpoint.path}\` | \`${endpoint.file}\` | ${endpoint.critical ? "yes" : "no"} |`).join("\n") +
  `\n\n## Public pages\n\n| Path | Page |\n|---|---|\n` +
  pages.map((page) => `| \`${page.path}\` | \`${page.file}\` |`).join("\n") +
  `\n\n## Environment variables\n\n| Variable | Tracked references |\n|---|---|\n` +
  [...envReferences].sort(([a], [b]) => a.localeCompare(b)).map(([name, refs]) => `| \`${name}\` | ${[...refs].sort().map((file) => `\`${file}\``).join("<br>")} |`).join("\n") +
  `\n\n## Migration artifacts\n\n${migrationFiles.map((file) => `- \`${file}\``).join("\n") || "- None"}\n\n` +
  `## Key configuration\n\n${keyConfigFiles.map((file) => `- \`${file}\``).join("\n")}\n`;
await Bun.write(join(outputRoot, "INVENTORY.md"), inventoryMarkdown);

const regressionMarkdown = `# Route regression matrix\n\n` +
  ...
  endpoints.map((endpoint) => {
    const auth = endpoint.path.startsWith("/api/auth/") ? "public/session mutation" : "preserve current guard";
    return `| ${endpoint.critical ? "critical" : "standard"} | ${endpoint.methods.join(", ")} | \`${endpoint.path}\` | ${auth} | status + response shape + side effects |`;
  }).join("\n") + `\n`;
await mkdir(join(root, "docs/testing"), { recursive: true });
await Bun.write(join(root, "docs/testing/route-regression-matrix.md"), regressionMarkdown);
