...
async function openComposer(page: Page) {
  ...
  await expect(page.locator(".aui-lexical-placeholder")).toHaveText("Posez une question, @ pour les fichiers, / pour les commandes");
...
test("matches the HonoUI composer structure across responsive widths", async ({ page }) => {
  ...
  await expect(page.getByRole("button", { name: "Ajouter du contexte ou changer de mode" })).toHaveCSS("cursor", "pointer");
  await expect(page.getByRole("button", { name: "Choisir un modèle" })).toContainText("claude-haiku-4-5");
  await expect(page.getByRole("button", { name: "Choisir l’effort" })).toContainText(/Moyen/i);
  await page.getByRole("button", { name: "Choisir un modèle" }).click();
...
test("sends selected model, effort and speed in the first session.create", async ({ page }) => {
  ...
  await expect(page.getByRole("button", { name: "Choisir un modèle" })).toContainText("claude-opus-4-6");
  ...
  await page.getByRole("button", { name: "Choisir l’effort" }).click();
  await expect(page.getByRole("button", { name: /^Très élevé/ })).toHaveCount(0);
  await page.getByRole("button", { name: /^Max/ }).click();
  await page.getByRole("button", { name: "Ajouter du contexte ou changer de mode" }).click();
  const speedMenu = page.getByRole("button", { name: "Vitesse", exact: true });
  await expect(speedMenu).toBeVisible();
  await expect(page.getByRole("menuitemradio", { name: "Rapide", exact: true })).toBeHidden();
  await speedMenu.hover();
  await page.getByRole("menuitemradio", { name: "Rapide", exact: true }).click();
...
test("saves an existing thread model through the same inference path as Settings", async ({ page }) => {
  ...
  await expect(page.getByRole("button", { name: "Choisir un modèle" })).toContainText("gpt-5.5");
  await page.getByRole("button", { name: "Choisir un modèle" }).click();
...
test("revalidates the saved model immediately when returning to chat without a hard refresh", async ({ page }) => {
  ...
  await expect(page.getByRole("button", { name: "Choisir un modèle" }))
...
test("uses native Hermes plan mode and the web-search turn instruction", async ({ page }) => {
  const calls = await openComposer(page);
  await page.getByRole("button", { name: "Ajouter du contexte ou changer de mode" }).click();
  const planModeSwitch = page.getByRole("switch", { name: "Mode plan" });
...
test("updates active-session effort and permissions through config.set", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "Choisir l’effort" }).click();
  await page.getByRole("button", { name: /^Bas/ }).click();
...
test("hides and omits effort for a model without configurable reasoning", async ({ page }) => {
  ...
  await expect(page.getByRole("button", { name: "Choisir l’effort" })).toHaveCount(0);
...
test("edits a user turn from the action bar", async ({ page }) => {
  ...
  await userMessage.locator('button[aria-label="Modifier"]').click();
...
test("reloads the assistant turn from the action bar", async ({ page }) => {
  ...
  await page
    .locator('[data-slot="aui_assistant-message-root"] button[aria-label="Régénérer"]')
    .click();
  await expect.poll(() => calls.filter((call) => call.method === "prompt.submit").length).toBe(2);
...
test("streams reasoning before assistant text", async ({ page }) => {
  ...
  await expect(page.locator("[data-slot=aui-streaming-status]")).toHaveCount(0);
  await expect(page.getByText("Réponse Hermes simulée.")).toBeVisible();
});
...
test("cancels a running Hermes turn and exposes real context usage", async ({ page }) => {
  ...
  await expect(page.getByRole("button", { name: "Arrêter la génération" })).toBeVisible();
