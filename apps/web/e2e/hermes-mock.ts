...
function resultFor(call: RpcCall, activeSessionInfo: MockSessionInfo = sessionInfo) {
  switch (call.method) {
    ...
    case "complete.path":
      return {
        items: [
          { value: "@file:apps/web/src/app/page.tsx", description: "Source file" },
          { value: "@folder:apps/web/src/components", description: "Components folder" },
        ],
      };
    case "complete.slash":
      return {
...
export async function installHermesMock(
  ...
  await page.routeWebSocket(/^(?:ws:\/\/127\.0\.0\.1:8787|wss:\/\/relay\.example\.test)(?:\/|\?)/, (ws) => {
    ...
    ws.onMessage((raw) => {
      ...
      } else if (call.method === "session.context_breakdown") {
        result = options.estimatedContext === false
          ? { context_used: 0, context_max: 128_000, estimated_total: 0, model: "gpt-5.5" }
          : { context_used: 33_700, context_max: 128_000, estimated_total: 33_700, model: "gpt-5.5" };
      } else {
