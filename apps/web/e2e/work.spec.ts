...
test("creates projects, agent teams and traceable automations from Work", async ({
  ...
  await expect(page.getByRole("region", { name: "Colonne Actif" }).getByText(`Tâche automatique ${suffix}`)).toBeVisible();
...
test("keeps Work board-only and persists vertical card reordering", async ({ page }) => {
  ...
  const activeColumn = page.getByRole("region", { name: "Colonne Actif" });
...
test("moves a backlog card into an empty column by drag-and-drop", async ({ page }) => {
  ...
  const todoColumn = page.getByRole("region", { name: "Colonne Prêt" });
  const activeColumn = page.getByRole("region", { name: "Colonne Actif" });
  ...
  await page.reload();
  await expect(page.getByRole("region", { name: "Colonne Prêt" }).locator("li")).toHaveCount(1);
});
