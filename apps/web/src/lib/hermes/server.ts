...
export { HermesRuntimeError } from "@/lib/hermes/runtime-error";
...
export async function runHermesInstallationCommand(
  ...
  const signed = await createGatewayServiceHeaders({
    method: "POST",
...
export async function runHermesUpgradeCommand(
  ...
  const signed = await createGatewayServiceHeaders({ method: "POST", requestUri: "/v1/control/upgrade", profile: input.profile, installationKey: installation.installationKey, body });
  try {
...
/**
 * `SOUL.md` of a profile — the identity file Hermes injects into every system
 * prompt. Path-scoped on the runtime, so the Edge compares the segment against
 * the ticket profile (`gateway/policy.go`) instead of injecting it into the body.
 */
export function readHermesProfileSoul(profile: string, scope: HermesRuntimeScope = {}) {
  return hermesFetch<{ content: string; exists: boolean }>(
...
export function writeHermesProfileSoul(profile: string, content: string, scope: HermesRuntimeScope = {}) {
  return hermesFetch<{ ok: boolean }>(
    `/api/profiles/${encodeURIComponent(profile)}/soul`,
...
/** The profile's role description, which Hermes uses as a delegation routing signal. */
export function writeHermesProfileDescription(
  profile: string,
  description: string,
  scope: HermesRuntimeScope = {},
) {
  return hermesFetch<{ ok: boolean; description: string }>(
