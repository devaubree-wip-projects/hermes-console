...
import { GATEWAY_PATHS, GATEWAY_PROTOCOL_VERSION } from "@hermes-console/shared/gateway";
import type { RuntimeInstallationStatus } from "@/db/schema";
import { createGatewayServiceHeaders, type GatewayAuthScope } from "@/lib/hermes/gateway-auth";

export const SUPPORTED_GATEWAY_PROTOCOL = GATEWAY_PROTOCOL_VERSION;
...
/**
 * @param scope `unregistered` quand l'installation n'est pas encore en base — c'est le
 * cas du flux « Connecter », qui sonde un Edge avant de l'enregistrer.
 */
export async function probeGateway(
  gatewayUrl: string,
  installationKey: string,
  scope: GatewayAuthScope = "installation",
): Promise<GatewayPreflightResult> {
  const [health, capabilities, ready] = await Promise.all([
    fetchWithTimeout(`${gatewayUrl}/healthz`),
    fetchWithTimeout(`${gatewayUrl}${GATEWAY_PATHS.capabilities}`),
  ...
  const requestUri = GATEWAY_PATHS.preflight;
  const preflight = await fetchWithTimeout(`${gatewayUrl}${requestUri}`, {
    headers: await createGatewayServiceHeaders({
      method: "GET",
      requestUri,
      profile: "default",
      installationKey,
...
export async function testGatewayProfile(
  gatewayUrl: string,
  installationKey: string,
  profile: string,
  scope: GatewayAuthScope = "installation",
) {
  const requestUri = GATEWAY_PATHS.profileTest;
  const body = JSON.stringify({ profile });
  const response = await fetchWithTimeout(`${gatewayUrl}${requestUri}`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      ...await createGatewayServiceHeaders({
        method: "POST",
        requestUri,
        profile,
        installationKey,
