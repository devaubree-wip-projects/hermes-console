...
import {
  ...
  profileGatewayRunningFromStatus,
...
describe("profile-scoped messaging state", () => {
  ...
  // Payloads relevés sur /api/status?profile=… en production (Hermes 0.19.0).
  test("reads a live profile gateway straight from the status payload", () => {
    expect(profileGatewayRunningFromStatus("atelier-lumiere-principal", {
      gateway_running: true,
      gateways: [{ profile: "default" }, { profile: "atelier-lumiere-principal" }],
    })).toBe(true);
  });

  test("reports a stopped profile gateway even while the runtime answers", () => {
    expect(profileGatewayRunningFromStatus("atelier-lumiere-reviewer", {
      gateway_running: false,
      gateways: [{ profile: "default" }, { profile: "atelier-lumiere-principal" }],
    })).toBe(false);
  });

  test("stays undecided when the payload carries no gateway signal at all", () => {
    expect(profileGatewayRunningFromStatus("atelier-lumiere-principal", {})).toBeNull();
  });
