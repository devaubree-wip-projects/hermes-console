...
const EFFORT_LOW: ReasoningControlOption = {
  id: "low",
  name: "Bas",
  description: "Réponses rapides, raisonnement léger",
};

const EFFORT_MEDIUM: ReasoningControlOption = {
  id: "medium",
  name: "Moyen",
  description: "Équilibre vitesse, coût et profondeur",
};

const EFFORT_HIGH: ReasoningControlOption = {
  id: "high",
  name: "Élevé",
  description: "Raisonnement plus profond pour le travail complexe",
};

const EFFORT_XHIGH: ReasoningControlOption = {
  id: "xhigh",
  name: "Très élevé",
  description: "Travail agentique et sessions longues",
};

const EFFORT_MAX: ReasoningControlOption = {
  id: "max",
  name: "Max",
  description: "Effort maximal supporté par ce modèle",
};
...
const OPENAI_REASONING_MODEL = /^(?:gpt-5(?:\.[0-9]+)?|codex|o[134](?:-|$))/;
...
export function formatEffortPreferenceLabel(
  provider: string,
  modelId: string,
  effort: string | undefined,
  supportsReasoning = true,
) {
  const config = getReasoningControlConfig(provider, modelId, supportsReasoning);
  if (!config || !effort) return null;
  const option = config.options.find((item) => item.id === effort);
  if (!option) return null;
  return `${config.label} · ${option.name}`;
}
