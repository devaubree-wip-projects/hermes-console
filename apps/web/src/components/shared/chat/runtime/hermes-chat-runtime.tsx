...
import {
  agentCreateOutcome,
  type AgentCreateConfirmationState,
  type AgentCreateResponse,
} from "@/components/shared/chat/runtime/agent-create-confirmation";
import {
...
function useHermesThreadRuntime(
  ...
  onAgentCreateRequest: (prompt: string) => void,
  ...
  const onNew = useCallback(async (message: AppendMessage) => {
    ...
    const agentRequest = parseAgentCreateRequest(text);
    if (agentRequest) {
      ...
      onAgentCreateRequest(agentRequest.prompt);
      return;
    }
  ...
  }, [manager, onAgentCreateRequest, remoteId, threadId]);
...
export function useHermesChatRuntime({
  ...
  const [agentCreateConfirmation, setAgentCreateConfirmation] =
    useState<AgentCreateConfirmationState | null>(null);
  const agentCreateConfirmationRef = useRef<AgentCreateConfirmationState | null>(null);
  const submittingAgentCreateRef = useRef<string | null>(null);
  const updateAgentCreateConfirmation = useCallback(
    (next: AgentCreateConfirmationState | null) => {
      agentCreateConfirmationRef.current = next;
      setAgentCreateConfirmation(next);
    },
    [],
  );
  ...
  const handleAgentCreateRequest = useCallback((prompt: string) => {
    if (!activeAgentId) {
      toast.error("Création d’agent", {
        description: "L’agent source actif est introuvable.",
      });
      return;
    }
    updateAgentCreateConfirmation({
      phase: "review",
      proposal: agentCreateRequestPayload(prompt, {
        sourceAgentId: activeAgentId,
        idempotencyKey: crypto.randomUUID(),
      }),
    });
  }, [activeAgentId, updateAgentCreateConfirmation]);

  const cancelAgentCreation = useCallback(() => {
    if (submittingAgentCreateRef.current) return;
    updateAgentCreateConfirmation(null);
  }, [updateAgentCreateConfirmation]);

  const confirmAgentCreation = useCallback(async () => {
    const current = agentCreateConfirmationRef.current;
    if (!current || !["review", "error"].includes(current.phase)) return;
    if (submittingAgentCreateRef.current === current.proposal.idempotencyKey) return;

    submittingAgentCreateRef.current = current.proposal.idempotencyKey;
    updateAgentCreateConfirmation({ phase: "submitting", proposal: current.proposal });
    try {
      const response = await fetch(agentsEndpoint, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify(current.proposal),
      });
      const body = await response.json().catch(() => null) as
        | (AgentCreateResponse & { error?: string })
        | null;
      if (!response.ok) {
        updateAgentCreateConfirmation({
          phase: "error",
          proposal: current.proposal,
          error: body?.error ?? `Agents API ${response.status}`,
          ...(body ? { result: body } : {}),
        });
        return;
      }
      ...
      const outcome = agentCreateOutcome(result);
      if (outcome.tone === "error") {
        ...
        toast.info(outcome.title, { description: outcome.description });
      }

      // A redirect is an explicit permission from the backend. In particular,
      // never infer one from the returned agent when its runtime is broken.
      ...
      submittingAgentCreateRef.current = null;
    }
  }, [agentsEndpoint, router, updateAgentCreateConfirmation]);

  const runtime = useRemoteThreadListRuntime({
    runtimeHook: function useHermesRuntimeHook() {
      return useHermesThreadRuntime(
        manager,
        handleAgentCreateRequest,
