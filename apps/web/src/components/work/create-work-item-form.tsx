...
export function CreateWorkItemForm({
  ...
  return (
    <Dialog open={expanded} onOpenChange={(open) => !submitting && setExpanded(open)}>
      ...
      <DialogContent className="w-[calc(100vw-2rem)] sm:max-w-[42rem]">
        <DialogHeader>
        ...
        <form id="create-work-item" onSubmit={submit} className="grid gap-4 sm:grid-cols-[minmax(0,1fr)_15rem]">
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Textarea id="work-description" value={description} onChange={(event) => setDescription(event.target.value)} rows={5} maxLength={40_000} placeholder="Décrivez les livrables, contraintes et ressources utiles." disabled={submitting} />
            </div>
          </div>
        ...
        <DialogFooter>
          <Button type="button" variant="ghost" disabled={submitting} onClick={() => setExpanded(false)}>Annuler</Button>
          <Button type="submit" form="create-work-item" disabled={submitting || !title.trim()}>{submitting ? <Loader2Icon className="animate-spin" /> : null}Créer</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
