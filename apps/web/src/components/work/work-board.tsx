...
import type { AssignOptions } from "@/components/work/assign-work-item-control";
...
import { WorkRunStatusBadge, WORK_ITEM_LABELS } from "@/components/work/work-status-badge";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
const columns: BoardColumn[] = [
  ...
  { status: "todo", label: WORK_ITEM_LABELS.todo, hint: "Assignée, prête à démarrer" },
  { status: "in_progress", label: WORK_ITEM_LABELS.in_progress, hint: "Hermes exécute" },
  { status: "blocked", label: WORK_ITEM_LABELS.blocked, hint: "Besoin humain ou dépendance" },
...
const statusLabels = WORK_ITEM_LABELS;
  ...
  return (
    <>
      <div className="flex items-center gap-1.5 pr-12">
        ...
        {item.executionState !== "idle" ? <span className="ml-auto"><WorkRunStatusBadge status={item.executionState} /></span> : null}
      </div>
      <button
        type="button"
...
function KanbanColumn({ column, items, canEdit, movingId, onMove, onOpen, onPrefetch }: {
  ...
  return (
    <section ref={setNodeRef} aria-label={`Colonne ${column.label}`} className={cn("flex h-full min-h-72 w-[280px] shrink-0 flex-col rounded-xl border border-transparent bg-muted/45 p-2 transition-colors", isOver && "border-primary/35 bg-primary/5")}>
      <div className="flex items-center gap-2 px-1.5 pb-2 pt-1" title="Réorganisez. Pour lancer Hermes : assignez un agent.">
...
export function WorkBoard({ apiBase, taskBase, items, canEdit, assignOptions }: { apiBase: string; taskBase: string; items: BoardItem[]; canEdit: boolean; assignOptions: AssignOptions }) {
  ...
  function preparePlacement(activeItemId: string, targetColumn: WorkItemStatus, overId: string, explicitStatus?: WorkItemStatus) {
    ...
    const sourceColumn = active.status;
    ...
    <>
      ...
      {selectedTaskId ? (
        <TaskDetailSheet onClose={() => setSelectedTaskId(null)}>
          <ClientTaskDetail key={selectedTaskId} apiBase={apiBase} workspaceBase={taskBase.slice(0, -"/tasks".length)} taskId={selectedTaskId} initialDetail={selectedTask?.detailSeed} canEdit={canEdit} assignOptions={assignOptions} onSelectTask={setSelectedTaskId} onDeleted={() => setSelectedTaskId(null)} />
        </TaskDetailSheet>
      ) : null}
    </>
  );
