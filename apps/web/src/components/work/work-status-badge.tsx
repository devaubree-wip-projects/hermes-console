...
// Single source of truth for the delivery-state labels, shared with the board.
export const WORK_ITEM_LABELS: Record<WorkItemStatus, string> = {
  backlog: "Backlog",
  todo: "Prêt",
  in_progress: "Actif",
  blocked: "Bloquée",
...
export function WorkItemStatusBadge({ status }: { status: WorkItemStatus }) {
  return <Badge variant="outline" className={stateClass(status)}>{WORK_ITEM_LABELS[status]}</Badge>;
}
