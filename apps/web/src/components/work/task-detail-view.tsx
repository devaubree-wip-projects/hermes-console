...
import { AlertTriangleIcon, ArrowLeftIcon, BotIcon, CalendarClockIcon, CheckCircle2Icon, CircleIcon, CircleSlash2Icon, Clock3Icon, ExternalLinkIcon, FolderKanbanIcon, GitBranchIcon, LinkIcon, MessageSquareTextIcon, PaperclipIcon, Trash2Icon } from "lucide-react";
import { Button } from "@/components/ui/button";
import { DeleteConfirmDialog } from "@/components/shared/delete-confirm-dialog";
import { formatDateTime } from "@/lib/format";
import { WorkLiveRefresh } from "@/components/work/work-live-refresh";
import { AssignWorkItemControl, type AssignOptions } from "@/components/work/assign-work-item-control";
...
export function TaskDetailView({ workspaceBase, detail, timeline, canEdit, assignOptions, embedded = false, hydrating = false, onSelectTask, onRefresh, onDeleted }: {
    ...
    <div className={embedded ? "flex h-full min-h-0 flex-col bg-background" : "min-h-full overflow-y-auto bg-background"}>
          ...
          <div className={`${embedded ? "" : "mt-4 "}flex flex-col gap-4 md:flex-row md:items-start md:justify-between`}>
            ...
            {canEdit && !hydrating ? (
              <div className="flex flex-wrap items-center gap-2">
                {item.status === "review" ? <ReviewActions endpoint={`${apiBase}/work-items/${item.id}`} onChanged={onRefresh} /> : null}
                <WorkRunActions apiBase={apiBase} workItemId={item.id} runId={currentRun?.id} active={runIsActive} canRun={Boolean(item.assigneeAgentId)} onChanged={onRefresh} />
                <DeleteConfirmDialog
                  endpoint={`${apiBase}/work-items/${item.id}`}
                  title="Supprimer la tâche"
                  description={<>La tâche <strong>{item.key}</strong> et son historique seront définitivement supprimées. Annulez d&apos;abord un run actif si nécessaire.</>}
            ...
            ) : hydrating && canEdit ? <div role="status" aria-label="Chargement des actions" className="h-9 w-28 animate-pulse rounded-md bg-muted motion-reduce:animate-none" /> : null}
          </div>
        </div>
      </header>
      ...
      <main className={`${embedded ? "min-h-0 flex-1 overflow-y-auto" : ""} grid w-full lg:grid-cols-[minmax(0,1fr)_21rem]`}>
        ...
        <aside className="divide-y divide-border/70 border-t border-border/80 bg-muted/25 px-5 lg:border-l lg:border-t-0">
          {canEdit && assignOptions && !hydrating ? <section className="py-6">
            <AssignWorkItemControl
              endpoint={`${apiBase}/work-items/${item.id}/assign`}
              options={assignOptions}
              current={{ type: item.assigneeType, agentId: item.assigneeAgentId, teamId: item.assigneeTeamId, userId: item.assigneeUserId, label: assignee?.name }}
              onChanged={onRefresh}
            />
          </section> : null}
