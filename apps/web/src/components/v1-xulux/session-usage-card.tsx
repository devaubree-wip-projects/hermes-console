...
import { formatEffortPreferenceLabel } from "@/components/shared/chat/constants/reasoning-config";
import { displayModelId } from "@/components/shared/chat/constants/openai-model-catalog";
...
function effortLabel(
  ...
  return formatEffortPreferenceLabel(provider ?? "", model ?? "", effort)
...
export function SessionUsageCard({
  ...
  const context = live?.context
    ?? live?.persistedContext
    ?? metrics?.context
    ?? live?.estimatedContext;
  const isEstimatedContext = context?.measurement === "estimated";
  ...
  return (
    ...
    >
      ...
      <div className="space-y-0.5" title={usageTitle}>
        <p className="text-[10px] text-muted-foreground">Tokens traités cumulés</p>
      ...
      <div className="space-y-1.5">
        ...
        <div
          aria-label={context
            ? `${numberFormatter.format(context.remainingTokens)} tokens de contexte restants${isEstimatedContext ? " selon l’estimation locale" : ""}`
          ...
          aria-valuetext={context
            ? `${percentFormatter.format(percent ?? 0)} pour cent utilisé, ${numberFormatter.format(context.remainingTokens)} tokens restants${isEstimatedContext ? ", estimation locale" : ""}`
            : "Mesure du contexte en attente"}
        ...
        <p className="text-right text-[10px] tabular-nums text-muted-foreground">
          {context
            ? `${numberFormatter.format(context.remainingTokens)} restants · ${percentFormatter.format(percent ?? 0)} %${isEstimatedContext ? " estimé" : ""}`
      ...
      <div className="grid gap-1 border-t pt-2 text-[10px] text-muted-foreground">
        <span className="flex min-w-0 items-center gap-1.5">
          ...
          <span className="truncate font-mono text-[10px]">{modelLabel(model)}</span>
        </span>
        <span className="flex min-w-0 items-center gap-1.5">
