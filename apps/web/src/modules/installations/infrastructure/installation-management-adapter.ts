...
import { invalidateInstallationSecrets } from "@/lib/hermes/installation-secrets";
  ...
  try {
    if (typeof body.archived === "boolean") {
      if (body.archived) {
        ...
        // Avant la révocation, pas après : cet appel est signé avec le secret de
        // l'installation, et le révoquer d'abord ferait retomber la signature sur le
        // secret dérivé, que l'Edge refuse. On perdrait l'invalidation des tickets
        // WebSocket encore ouverts, en silence.
        const ticketsRevoked = await revokeEdgeTickets({
          gatewayUrl: context.installation.gatewayUrl,
        ...
        const [installation] = await db.transaction(async (tx) => {
          ...
          // Le secret meurt avec l'identité. C'est le gain net du secret par
          // installation : jusqu'ici, couper l'accès d'un Edge imposait de tourner le
          // master d'environnement, donc de casser toutes les autres installations.
          // Aucune fenêtre de grâce ici, contrairement à une rotation : on veut
          // précisément que l'ancien secret cesse d'être accepté immédiatement.
          await tx.update(runtimeInstallationSecrets)
            .set({ status: "revoked", revokedAt: now })
            .where(and(
              eq(runtimeInstallationSecrets.installationId, installationId),
        ...
        // Le cache sert le secret quinze secondes : sans purge, la Console
        // continuerait d'accepter les requêtes d'un Edge qu'elle vient de couper.
        invalidateInstallationSecrets(installation.installationKey);
        // Signé avec le master (portée « relay »), donc insensible à la révocation
        // ci-dessus — l'ordre importe seulement pour l'appel aux tickets.
