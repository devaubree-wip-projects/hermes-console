...
export const drizzleInstallationRepository: InstallationRepositoryPort = {
  ...
  classifyError(error) {
    ...
    // La clé est unique globalement (elle résout le secret de l'installation), donc le
    // conflit remonte sur l'index global — souvent parce qu'elle appartient à un autre
    // tenant, ce que le message ne doit surtout pas révéler.
    const duplicate = message.includes("runtime_installations_key_uidx")
      || message.includes("runtime_installations_tenant_key_uidx");
    if (duplicate) return { message: "Cette clé d’installation existe déjà.", status: 409 };
    // Un Edge déjà enrôlé possède un secret propre que ce flux ne connaît pas : il
    // signe avec le secret dérivé, l'Edge répond 401. Sans cette traduction,
    // l'utilisateur lit « le Edge a refusé l'identité » sans savoir quoi faire.
    if (message.includes("refusé l’identité")) {
      return {
        message: "Ce Edge possède déjà une identité d’enrôlement : utilisez l’onglet « Enrôler ».",
        status: 409,
      };
    }
