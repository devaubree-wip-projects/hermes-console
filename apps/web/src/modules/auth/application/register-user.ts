...
export function createRegisterUser(dependencies: {
  ...
  /** Vrai pour les comptes qui exploitent la Console — voir `lib/operator`. */
  isOperatorEmail: (email: string) => boolean;
}) {
  return async function registerUser(input: RegisterInput) {
    ...
    // La Console héberge les données de plusieurs entreprises : on n'y entre pas
    // en remplissant un formulaire. Le contrôle porte sur l'adresse invitée et
    // non sur la possession du lien, parce que la suite du parcours — accepter
    // l'invitation — vérifie déjà le jeton.
    const invite =
      dependencies.isOperatorEmail(email) ||
      (await dependencies.repository.hasPendingInvitation(email));
    if (!invite) {
      throw new AuthApplicationError(
        403,
        "La création de compte se fait sur invitation. Demandez à votre organisation de vous inviter.",
      );
    }
