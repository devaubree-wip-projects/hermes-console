...
describe("Work control plane PostgreSQL integration", () => {
  databaseTest(
    ...
    async () => {
      ...
      try {
        ...
        // A promoted step inherits the parent's assignee: it must not become an
        // ownerless, unstartable backlog card.
        expect(promoted.assigneeAgentId).toBe(agent.id);
        ...
        if (tenantId) {
          ...
          // Clear assignees before removing agents/teams: assignee_{agent,team}_id
          // are ON DELETE SET NULL, which would violate work_items_assignee_check
          // while the assignee_type column still reads 'agent'/'team'.
          await db
  ...
  databaseTest(
    "a cancelled task stays cancelled and stops its run even if the run completes late",
    async () => {
      const [{ db }, schema, work, runtime] = await Promise.all([
        import("@/db"),
        import("@/db/schema"),
      ...
      const [user] = await db
        .insert(schema.users)
        .values({
          email: `work-cancel-${suffix}@hermes.local`,
          passwordHash: "integration-test-only",
          name: "Work cancel",
        })
        .returning();
      ...
      try {
        const [tenant] = await db
          .insert(schema.tenants)
          .values({
            name: `Work cancel ${suffix}`,
            slug: `work-cxl-${suffix}`,
            ownerUserId: user.id,
          })
          .returning();
        tenantId = tenant.id;
        ...
        const [installation] = await db
          ...
          .values({
            tenantId: tenant.id,
            name: "Cancel Edge",
            installationKey: `cancel-edge-${suffix}`,
            origin: "local_managed",
            managementLevel: "managed",
            transport: "direct",
            gatewayUrl: "http://127.0.0.1:8787",
        ...
        // Scenario 1 — the item is cancelled while its run is still active (e.g. a
        // status change from the detail view, which does NOT stop the run). A run
        // that finishes afterwards must not resurrect the task nor spam the inbox.
        const cancelledDuringRun = await work.createWorkspaceWorkItem({
          context,
          title: "Annulée pendant l’exécution",
          description: "Le run finit après l’annulation et ne doit pas ressusciter la tâche.",
          reviewPolicy: "required",
          assignee: { type: "agent", agentId: agent.id },
        });
        if (!cancelledDuringRun.run) throw new Error("Expected an enqueued run.");
        const [claim1] = await runtime.claimWorkRuns({
          installationId: installation.id,
          edgeId: "edge-cxl-1",
          capacity: 1,
        });
        await runtime.startWorkRun({
          ...
          hermesSessionId: "session-cxl-1",
        });
        await work.updateWorkspaceWorkItem({
          context,
          workItemId: cancelledDuringRun.item.id,
          status: "cancelled",
        });
        const [runStillActive] = await db
          .select()
          .from(schema.workRuns)
          .where(eq(schema.workRuns.id, cancelledDuringRun.run.id));
        expect(runStillActive.status).toBe("running");
        await runtime.completeWorkRun({
          runId: claim1.runId,
          installationId: installation.id,
          leaseToken: claim1.leaseToken,
          status: "succeeded",
          resultSummary: "Livrable arrivé trop tard.",
        });
        const [notResurrected] = await db
          .select()
          .from(schema.workItems)
          .where(eq(schema.workItems.id, cancelledDuringRun.item.id));
        expect(notResurrected.status).toBe("cancelled");
        const lateInbox = await db
          .select()
          .from(schema.inboxItems)
          .where(eq(schema.inboxItems.sourceId, cancelledDuringRun.run.id));
        expect(lateInbox).toHaveLength(0);

        // Scenario 2 — cancelling from the board must also STOP the run, not just
        // relabel the card (placeWorkspaceWorkItem cancels active runs).
        const boardCancelled = await work.createWorkspaceWorkItem({
          context,
          title: "Annulée depuis le board",
          description: "Glisser vers Annulées doit stopper l’agent.",
          reviewPolicy: "none",
          assignee: { type: "agent", agentId: agent.id },
        });
        if (!boardCancelled.run) throw new Error("Expected an enqueued run.");
        const [claim2] = await runtime.claimWorkRuns({
          installationId: installation.id,
          edgeId: "edge-cxl-2",
          capacity: 1,
        });
        await runtime.startWorkRun({
          ...
          hermesSessionId: "session-cxl-2",
        });
        await work.placeWorkspaceWorkItem({
          context,
          workItemId: boardCancelled.item.id,
          status: "cancelled",
          previousItemId: null,
          nextItemId: null,
        });
        const [stoppedRun] = await db
          .select()
          .from(schema.workRuns)
          .where(eq(schema.workRuns.id, boardCancelled.run.id));
        expect(stoppedRun.status).toBe("cancelling");
        const [boardItem] = await db
          .select()
          .from(schema.workItems)
          .where(eq(schema.workItems.id, boardCancelled.item.id));
        expect(boardItem.status).toBe("cancelled");
      } finally {
        if (tenantId) {
          const tenantWorkspaces = db
            .select({ id: schema.workspaces.id })
