...
export const MAX_WORK_RESOURCES_PER_SCOPE = 16;
...
export async function enqueueWorkRun(input: {
  ...
  // Pas de run sans matière (V1 §3). Runs from delegation or rerun already
  // cleared the gate on their originating run, so only fresh human/automation
  // triggers pass through it. Placed after the idempotency short-circuit so a
  // replay never re-gates a run that was already accepted.
  if (
    input.triggerType === "assignment" ||
    input.triggerType === "mention" ||
    input.triggerType === "automation" ||
    input.triggerType === "api"
  ) {
    const projectId = item.projectId;
    const [counts] = await db
      .select({
        linked: sql<number>`count(*) filter (where ${workResources.workItemId} = ${item.id})`,
        project: projectId
          ? sql<number>`count(*) filter (where ${workResources.projectId} = ${projectId})`
          : sql<number>`0`,
      })
      .from(workResources)
      .where(
    ...
    assertRunnableBrief({
      ...
      linkedResourceCount: Number(counts?.linked ?? 0),
      projectResourceCount: Number(counts?.project ?? 0),
    });
  }
  ...
  const item = await db.transaction(async (tx) => {
    ...
    await tx.insert(auditEvents).values({
      ...
      metadata: {
        ...(input.auditMetadata ?? {}),
        key: created.key,
        assigneeType: created.assigneeType,
      },
    });
...
export async function listWorkspaceWorkItems(input: {
  ...
  const rows = await db
    .select({
      ...
      latestRunStatus: sql<
        WorkRunStatus | null
      >`(array_agg(${workRuns.status} order by ${workRuns.createdAt} desc))[1]`,
    })
    .from(workItems)
  ...
  // executionState is the live "is Hermes working?" projection (V1 §3, axis B),
  // additive to the response — the kanban column stays the delivery_state.
  return rows.map(({ latestRunStatus, ...row }) => ({
    ...row,
    executionState: deriveWorkExecutionState({
...
export async function getWorkspaceWorkItem(
  ...
  const activeRun =
    ...
    primaryRuns[0] ??
    null;
  const executionState = deriveWorkExecutionState({
    activeRunCount: primaryRuns.filter((run) => !workRunTerminal(run.status))
      .length,
    latestRunStatus: primaryRuns[0]?.status ?? null,
  });
  const [
    steps,
    comments,
...
function boardStatusGroup(status: WorkItemStatus): WorkItemStatus[] {
  // Blocked is its own board column now, so each status owns its own ordering
  // group — drag placement and display must not diverge.
  return [status];
}
...
export async function placeWorkspaceWorkItem(input: {
  ...
  const result = await db.transaction(async (tx) => {
    ...
    if (input.status === "cancelled") {
      // Cancelling from the board must also stop the agent, not just relabel the
      // card: otherwise the run keeps burning and could resurrect the task.
      await tx
        .update(workRuns)
        .set({ status: "cancelled", completedAt: occurredAt, updatedAt: occurredAt })
        .where(and(eq(workRuns.workItemId, current.id), eq(workRuns.status, "queued")));
      await tx
        .update(workRuns)
        .set({ status: "cancelling", updatedAt: occurredAt })
        .where(and(eq(workRuns.workItemId, current.id), inArray(workRuns.status, ["preparing", "running", "waiting_input"])));
    }
    await tx.insert(auditEvents).values({
      tenantId: input.context.tenantId,
      workspaceId: input.context.workspaceId,
...
export async function promoteWorkspacePlanStep(input: {
  ...
  // Inherit the parent's assignee so a promoted step is executable (or at least
  // reassignable) instead of a dead-end backlog card with no owner.
  const parentAssignee = step.item.assigneeType
    ? {
        type: step.item.assigneeType,
        userId: step.item.assigneeUserId,
        agentId: step.item.assigneeAgentId,
        teamId: step.item.assigneeTeamId,
      }
    : undefined;
...
function validateWorkResourceUri(kind: WorkResourceKind, value: string) {
  ...
  if (kind === "file" && uri.startsWith("grant://")) {
    if (
      !/^grant:\/\/[a-zA-Z0-9][a-zA-Z0-9_-]{0,63}\/[a-zA-Z0-9][a-zA-Z0-9._/-]*$/.test(
        uri,
      ) ||
      uri.includes("..") ||
      uri.includes("\\")
      ...
      throw new WorkDomainError(
        "invalid_resource_uri",
        "Le grant doit utiliser grant://alias/chemin sans traversée.",
      );
    }
    return uri;
  }
...
export function minimumRoleForWorkResource(
  kind: WorkResourceKind,
  uri: string,
): MembershipRole {
  return kind === "file" && uri.trim().startsWith("grant://")
    ? "owner"
    : "member";
}
...
export async function createWorkspaceWorkResource(input: {
  ...
  if (
    minimumRoleForWorkResource(input.kind, input.uri) === "owner"
    && input.context.role !== "owner"
  ) {
    throw new WorkDomainError(
      "grant_owner_required",
      "Seul un Owner peut autoriser un accès au filesystem de l’Edge.",
    );
  }
  const [scopeUsage] = await db
    .select({ total: count() })
    .from(workResources)
    .where(
      and(
        eq(workResources.workspaceId, input.context.workspaceId),
        input.workItemId
          ? eq(workResources.workItemId, input.workItemId)
          : eq(workResources.projectId, input.projectId!),
      ),
    );
  if ((scopeUsage?.total ?? 0) >= MAX_WORK_RESOURCES_PER_SCOPE) {
    throw new WorkDomainError(
      "resource_limit_reached",
      `Une tâche ou un projet accepte au maximum ${MAX_WORK_RESOURCES_PER_SCOPE} ressources.`,
    );
  }
  const uri = validateWorkResourceUri(input.kind, input.uri);
  const metadata = sanitizeResourceMetadata(input.metadata);
  if (input.kind === "file" && uri.startsWith("work://resources/")) {
    const fileId = typeof metadata.fileId === "string" ? metadata.fileId : "";
    const [file] = UUID_PATTERN.test(fileId)
      ? await db
          .select({ id: files.id })
          .from(files)
          .where(
            and(
              eq(files.id, fileId),
              eq(files.workspaceId, input.context.workspaceId),
            ),
          )
          .limit(1)
      : [];
    if (!file) {
      throw new WorkDomainError(
        "invalid_file_resource",
        "Le fichier source est absent de cette organisation.",
      );
    }
  }
...
export async function listWorkspaceInbox(input: {
  ...
  return db
    .select({
      ...
      reason: inboxItems.reason,
      readAt: inboxItems.readAt,
      createdAt: inboxItems.createdAt,
      // work_run inbox items carry the run id as sourceId; resolve it to the
      // parent work item so the inbox can deep-link to /tasks?task=<id>.
      targetWorkItemId: workRuns.workItemId,
    })
    .from(inboxItems)
    .leftJoin(
      workRuns,
      // Cast only work_run rows: sourceId is text and other inbox types must
      // never be forced through ::uuid during join evaluation.
      eq(
        workRuns.id,
        sql`(case when ${inboxItems.sourceType} = 'work_run' then ${inboxItems.sourceId}::uuid else null end)`,
      ),
    )
    .where(
      and(
...
export async function countWorkspaceInboxUnread(input: {
  workspaceId: string;
  userId: string;
}) {
  const [row] = await db
...
export async function deleteWorkspaceAgentTeam(input: {
  ...
  return db.transaction(async (tx) => {
    // Automations require a non-null assignee (work_automations_assignee_check),
    // so a team still driving one cannot be silently unassigned.
    const [assignedAutomation] = await tx
      .select({ id: workAutomations.id })
      .from(workAutomations)
      .where(
        and(
          eq(workAutomations.workspaceId, input.context.workspaceId),
          eq(workAutomations.assigneeTeamId, input.teamId),
        ),
      )
      .limit(1);
    if (assignedAutomation)
      throw new WorkConflictError(
        "Cette équipe est assignée à une automatisation. Réassignez-la avant de supprimer l’équipe.",
      );
    // Members cascade. Clear work-item assignments first: assignee_team_id is
    // ON DELETE SET NULL and would otherwise violate work_items_assignee_check.
    await tx
      .update(workItems)
      .set({ assigneeType: null, assigneeTeamId: null, updatedAt: new Date() })
      .where(
        and(
          eq(workItems.workspaceId, input.context.workspaceId),
          eq(workItems.assigneeTeamId, input.teamId),
        ),
      );
    const [team] = await tx
      .delete(agentTeams)
...
export async function resolveWorkspaceIntervention(input: {
  ...
  // A resolved intervention must stop showing up as an unread action for every
  // recipient, not just the user who resolved it.
  await db
    .update(inboxItems)
    .set({ readAt: new Date() })
    .where(
      and(
        eq(inboxItems.workspaceId, input.context.workspaceId),
        eq(inboxItems.sourceType, "work_intervention"),
        eq(inboxItems.sourceId, intervention.id),
      ),
    );
  return updated;
}
