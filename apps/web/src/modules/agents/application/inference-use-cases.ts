...
export function createInferenceUseCases(dependencies: InferenceDependencies) {
  return {
    async get(params: AgentContextParams) {
      ...
      try {
        const runtime = await dependencies.runtime.load(resolved.agent!.id, resolved.agent!.hermesProfileName);
    ...
    async update(params: AgentContextParams, body: Record<string, unknown> | null) {
      ...
      try {
        ...
        if (mode === "reasoning") {
          if (!REASONING_EFFORTS.has(reasoningEffort)) return result({ error: "Effort de raisonnement invalide." }, 400);
