...
export async function GET(
  ...
  const { access, agent } = await resolveAgent(tenantSlug, workspaceSlug, agentSlug);
...
export async function POST(request: Request, { params }: { params: Promise<{ tenantSlug: string; workspaceSlug: string; agentSlug: string }> }) {
  const { tenantSlug, workspaceSlug, agentSlug } = await params;
  const { user, access, agent } = await resolveAgent(tenantSlug, workspaceSlug, agentSlug);
  if (!access) return NextResponse.json({ error: "Workspace introuvable." }, { status: 404 });
