...
import { canApprove, getTenantAccessBySlug } from "@/lib/workspace";
...
export default async function ApprovalsPage({
  ...
  return (
    <div className="min-h-full overflow-y-auto bg-background">
      ...
      <main className="w-full space-y-8 px-5 py-6 md:px-8">
        {pending.length ? (
            ...
            {pending.map((entry) => (
              <article key={entry.id} className="rounded-xl border bg-card p-5">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  <Link
                    href={`${base}/tasks?task=${encodeURIComponent(entry.workItemId)}`}
                ...
                {canApprove(access.role) ? (
                  <div className="mt-4 border-t pt-4">
                    <InterventionActions
                      endpoint={`/api/${tenantSlug}/interventions/${entry.id}`}
                      type={entry.type}
                    />
                  </div>
                ) : null}
