...
import { InboxActions, InboxItemRow } from "@/components/work/inbox-actions";
...
export default async function InboxPage({
  ...
  return (
    <div className="min-h-full overflow-y-auto bg-background">
      <WorkLiveRefresh endpoint={`/api/${tenantSlug}/work-stream`} />
      <header className="border-b px-5 py-5 md:px-8">
        <div className="flex w-full items-end justify-between gap-4">
          ...
          <InboxActions
            endpoint={`/api/${tenantSlug}/inbox`}
            disabled={!unread.length}
          />
        </div>
      </header>
      <main className="w-full space-y-4 px-5 py-6 md:px-8">
        {items.length ? (
              ...
              const href =
                item.sourceType === "work_intervention"
                  ...
                  : item.sourceType === "work_item"
                    ? `${base}/tasks?task=${encodeURIComponent(item.sourceId)}`
                    : item.sourceType === "work_run"
                      ? item.targetWorkItemId
                        ? `${base}/tasks?task=${encodeURIComponent(item.targetWorkItemId)}`
                        : `${base}/tasks`
                      : item.sourceType === "work_automation"
                        ? `${base}/automations`
                        : `${base}/tasks`;
              return (
                <li key={item.id}>
                  ...
                  >
                    <span className="mt-1">
                      {item.readAt ? (
                        ...
                        <BellRingIcon className="size-4 text-amber-600" aria-hidden="true" />
                      )}
                      <span className="sr-only">{item.readAt ? "Lu" : "Non lu"}</span>
                    </span>
