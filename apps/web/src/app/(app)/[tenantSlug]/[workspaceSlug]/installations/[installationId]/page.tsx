...
export default async function InstallationDetailPage({
  ...
  const endpoint = `/api/${tenantSlug}/${workspaceSlug}/installations/${installation.id}`;
  ...
  return <div className="min-h-full overflow-y-auto bg-background" data-testid="installation-detail">
    <PageHeading
      actions={<Button asChild variant="outline"><Link href={`/${tenantSlug}/${workspaceSlug}/installations`}><ArrowLeftIcon />Toutes les installations</Link></Button>}
        ...
        <Tabs defaultValue="overview">
          ...
          <TabsContent className="grid gap-4 pt-4 md:grid-cols-2" value="overview">
            ...
            <Card><CardHeader><CardTitle className="flex items-center gap-2"><BotIcon />Agents et profils</CardTitle></CardHeader><CardContent className="grid gap-2">
              {assignedAgents.length ? assignedAgents.map((agent) => <div className="flex items-center justify-between rounded-lg border p-2" key={agent.id}><span>{agent.name}</span><Badge variant="secondary">{agent.profileName}</Badge></div>) : <p className="text-sm text-muted-foreground">Aucun agent de ce workspace n’est associé.</p>}
            </CardContent></Card>
          </TabsContent>
