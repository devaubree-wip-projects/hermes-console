...
export default async function ProjectDetailPage({ params }: { params: Promise<{ tenantSlug: string; workspaceSlug: string; projectId: string }> }) {
  const { tenantSlug, workspaceSlug, projectId } = await params;
  ...
  return <div className="min-h-full overflow-y-auto bg-background">
    <header className="border-b px-5 py-5 md:px-8"><div className="mx-auto max-w-6xl"><Link href={`${base}/projects`} className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground"><ArrowLeftIcon className="size-4" />Projets</Link><div className="mt-4 flex flex-wrap items-start justify-between gap-4"><div><span className="font-mono text-xs text-muted-foreground">{project.key}</span><h2 className="mt-1 text-2xl font-semibold tracking-tight">{project.name}</h2><p className="mt-2 max-w-[70ch] text-sm leading-6 text-muted-foreground">{project.description || "Aucun contexte projet."}</p></div><Badge variant="outline">{project.status}</Badge></div></div></header>
    <main className="mx-auto grid max-w-6xl gap-6 px-5 py-6 md:px-8 lg:grid-cols-[minmax(0,1fr)_20rem]">
      <section><div className="flex items-center justify-between gap-4"><h3 className="text-sm font-semibold">Tâches</h3><span className="font-mono text-xs text-muted-foreground">{progress.completed}/{progress.total} terminées</span></div>{tasks.length ? <ul className="mt-3 divide-y rounded-xl border bg-card">{tasks.map(({ item, assigneeAgentName, activeRunCount }) => <li key={item.id}><Link href={`${base}/tasks/${item.id}`} className="flex items-center justify-between gap-4 px-4 py-3 hover:bg-muted/40"><span className="min-w-0"><span className="font-mono text-[11px] text-muted-foreground">{item.key}</span><span className="block truncate text-sm font-medium">{item.title}</span><span className="text-xs text-muted-foreground">{assigneeAgentName ?? "Non assignée"}</span></span><span className="flex items-center gap-2"><WorkItemStatusBadge status={item.status} />{activeRunCount ? <CircleDotIcon className="size-3 text-blue-500" /> : null}</span></Link></li>)}</ul> : <p className="mt-3 rounded-xl border border-dashed p-8 text-center text-sm text-muted-foreground">Aucune tâche dans ce projet.</p>}</section>
