...
export const tenantInvitations = pgTable(
  ...
  (t) => [uniqueIndex("tenant_invitations_tenant_email_idx").on(t.tenantId, t.email)],
);
...
export const runtimeInstallations = pgTable(
  ...
  (t) => [
    // Globalement unique, pour deux raisons distinctes. D'abord parce que cette clé
    // désigne l'unique ligne de `runtime_installation_secrets` à charger : deux
    // installations homonymes rendraient la résolution ambiguë. Ensuite parce qu'une
    // installation sans secret propre retombe sur un secret DÉRIVÉ de cette clé seule
    // (`deriveInstallationSecret`) — deux tenants la partageant partageraient alors ce
    // secret, et chacun pourrait s'authentifier à la place de l'autre.
    uniqueIndex("runtime_installations_key_uidx").on(t.installationKey),
...
export type RuntimeSecretStatus = "active" | "superseded" | "revoked";

/**
 * Secrets HMAC qu'un Edge enrôlé partage avec la Console, chiffrés au repos
 * (`lib/hermes/secret-vault.ts`). Une ligne porte les DEUX secrets : l'enrôlement les
 * remet toujours ensemble, et les séparer permettrait de les désynchroniser.
 *
 * Table volontairement distincte de `runtime_identities` : celle-ci révoque toutes ses
 * lignes précédentes dès qu'une nouvelle identité arrive, ce qui interdirait la fenêtre
 * de grâce nécessaire pour qu'un Edge en cours de redémarrage continue d'être accepté.
 *
 * Une installation SANS ligne ici n'est pas cassée : elle retombe sur le secret dérivé
 * du master d'environnement. C'est ce qui rend la table adoptable sans reprise de
 * données, et il ne faut donc jamais la pré-remplir pour une installation existante —
 * la Console changerait de secret sans que l'Edge en soit informé.
 */
export const runtimeInstallationSecrets = pgTable(
  "runtime_installation_secrets",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    installationId: uuid("installation_id")
      .notNull()
      .references(() => runtimeInstallations.id, { onDelete: "cascade" }),
    serviceSecret: text("service_secret").notNull(),
    ticketSecret: text("ticket_secret").notNull(),
    // Numéro de la clé de chiffrement ayant scellé ces valeurs : permet un
    // re-chiffrement futur sans avoir à deviner laquelle a servi.
    keyVersion: integer("key_version").notNull().default(1),
    status: text("status").$type<RuntimeSecretStatus>().notNull().default("active"),
    activatedAt: timestamp("activated_at", { withTimezone: true })
      .notNull()
      .defaultNow(),
    supersededAt: timestamp("superseded_at", { withTimezone: true }),
    revokedAt: timestamp("revoked_at", { withTimezone: true }),
    // Null lorsque la génération naît d'un échange machine (enrôlement) plutôt que
  ...
  (t) => [
    // Partiel : une seule génération active par installation, mais autant de
    // générations `superseded`/`revoked` que d'historique.
    uniqueIndex("runtime_installation_secrets_active_uidx")
      .on(t.installationId)
      .where(sql`${t.status} = 'active'`),
    index("runtime_installation_secrets_installation_status_idx").on(
      t.installationId,
      t.status,
    ),
  ],
...
/**
 * Réglages d'instance surchargeant le `.env` du process Console.
 *
 * Volontairement globale et non rattachée à un tenant : ce sont des réglages du
 * déploiement (SMTP, journalisation, URL publique), et une Console sert une seule
 * instance. Les rattacher à un tenant laisserait croire qu'un espace de travail peut
 * choisir son propre serveur SMTP, ce qui n'est pas vrai.
 *
 * La base prime sur l'environnement — jamais l'inverse. L'env ne sert que de valeur
 * de repli quand aucune ligne n'existe, ce qui rend la table adoptable sans reprise.
 */
export const consoleSettings = pgTable("console_settings", {
  // Nom exact de la variable d'environnement, pour que la correspondance entre ce
  // qu'on lit ici et ce qu'on lirait dans le fichier soit littérale.
  key: text("key").primaryKey(),
  value: text("value"),
  // Scellé par `lib/hermes/secret-vault.ts`, AAD `setting:<key>`. Exclusif avec
  // `value` : un réglage secret n'a jamais de valeur en clair.
...
export const agents = pgTable(
  ...
  (t) => [
    ...
    uniqueIndex("agents_workspace_provisioning_key_uidx").on(
      t.workspaceId,
      t.provisioningIdempotencyKey,
    ),
  ],
);
...
export const workResources = pgTable(
  ...
  (t) => [
    ...
    index("work_resources_source_run_idx").on(t.sourceRunId),
  ],
);
...
/** Régime d'envoi déclaré par le tenant — voir `modules/mail/domain/mail-policy`. */
export type NatureDestinataire = "prospection" | "relation_client";

// Un relais d'envoi par fournisseur et par tenant. Le secret (mot de passe
// d'application Gmail, clé Brevo ou Resend) est scellé par le coffre : la Console
// l'utilise pour le compte du tenant sans jamais avoir à le réafficher.
export const mailCredentials = pgTable(
  "mail_credentials",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    tenantId: uuid("tenant_id")
    ...
    /** AES-256-GCM, contexte `mail-credential:<id>` — voir lib/hermes/secret-vault. */
    sealedSecret: text("sealed_secret").notNull(),
    /** Hôte, port, TLS et compte SMTP. Vide pour Brevo et Resend, qui n'ont qu'une clé. */
    transport: jsonb("transport").$type<Record<string, unknown>>(),
    // Gmail coupe autour de 500 envois par jour et suspend le compte au-delà :
    // la limite appartient au relais, pas au tenant.
  ...
  (t) => [
    uniqueIndex("mail_credentials_tenant_provider_uidx").on(t.tenantId, t.provider),
    // Un seul relais par défaut : sans cette garde, le choix dépendrait de l'ordre
    // de lecture, donc le message partirait un jour du mauvais domaine.
    uniqueIndex("mail_credentials_tenant_default_uidx")
      .on(t.tenantId)
      .where(sql`is_default`),
  ],
);

// Journal d'envoi : il sert à trois choses à la fois — ne pas écrire deux fois à
// la même personne, compter le quota du jour, et répondre à une demande d'accès.
export const mailSends = pgTable(
  "mail_sends",
  {
    ...
    provider: text("provider").$type<MailProvider>().notNull(),
    /** Toujours normalisé en minuscules : la déduplication en dépend. */
    recipient: text("recipient").notNull(),
    subject: text("subject").notNull(),
    // Ce que le tenant déclare de sa relation au destinataire. Conservé parce
    // que c'est lui qui répond de la déclaration : la piste doit dire sous quel
    // régime le message est parti, pas seulement qu'il est parti.
    nature: text("nature").$type<NatureDestinataire>().notNull().default("prospection"),
    /** L'avis public qui justifie la prise de contact — exigé en prospection, nul sinon. */
    sourceUrl: text("source_url"),
    providerMessageId: text("provider_message_id"),
    // `pending` est posé AVANT l'appel au relais : la ligne réserve l'adresse.
    // Sans cette réservation, deux exécutions concurrentes du même cron passent
    // toutes deux le contrôle de doublon et le prospect reçoit le message en double.
    status: text("status").$type<"pending" | "sent" | "failed">().notNull(),
    error: text("error"),
    createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
  },
  (t) => [
    index("mail_sends_tenant_recipient_idx").on(t.tenantId, t.recipient),
    index("mail_sends_tenant_created_idx").on(t.tenantId, t.createdAt),
    // Un échec ne brûle pas l'adresse : seuls `pending` et `sent` la retiennent.
    // Réservation limitée à la prospection : c'est là qu'un second message est
    // une faute. En relation client, la répétition est le service rendu, et cet
    // index l'interdirait dès la première relance.
    uniqueIndex("mail_sends_tenant_recipient_live_uidx")
      .on(t.tenantId, t.recipient)
      .where(sql`status <> 'failed' AND nature = 'prospection'`),
  ],
);

// Opposition à toute prospection. Une table plutôt qu'un fichier : le « STOP »
// promis dans chaque message doit survivre au redémarrage d'un conteneur.
export const mailSuppressions = pgTable(
  "mail_suppressions",
  {
    ...
    reason: text("reason").$type<"unsubscribe" | "bounce" | "manual">().notNull(),
    createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
  },
  (t) => [uniqueIndex("mail_suppressions_tenant_address_uidx").on(t.tenantId, t.address)],
);
...
export type RuntimeInstallationSecret =
  typeof runtimeInstallationSecrets.$inferSelect;
export type ConsoleSetting = typeof consoleSettings.$inferSelect;
...
export type MailCredential = typeof mailCredentials.$inferSelect;
