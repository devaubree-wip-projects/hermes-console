...
func main() {
	...
	// Loud rather than silent: without HERMES_WORK_INSTALLATION_ID the Console cannot
	// tell which installation this Edge speaks for, so Work stays off instead of
	// having every call rejected. The id is readable once onboarding has run.
	if config.Mode == "edge" && config.WorkControlPlaneURL != nil && config.WorkInstallationID == "" {
		logger.Warn("Work executor disabled: set HERMES_WORK_INSTALLATION_ID to the runtime_installations row this Edge serves, then restart")
	}
