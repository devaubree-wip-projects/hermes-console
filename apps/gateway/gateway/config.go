...
type Config struct {
	...
	// Délai des seules routes qui ouvrent une connexion MCP : le reste doit
	// continuer d'échouer vite sous UpstreamTimeout.
...
func LoadConfig() (Config, error) {
	...
	return Config{
		...
		// The installation id is part of the Work contract: the Console resolves the
		// row by primary key and rejects a request that does not name one. It only
		// exists after onboarding, so requiring it at boot would deadlock a fresh
		// install — Work simply stays off until it is configured (main.go says so).
		WorkEnabled:                  mode == "edge" && workControlPlaneURL != nil && workInstallationID != "" && !strings.EqualFold(env("HERMES_WORK_ENABLED", "true"), "false"),
		...
		MCPProbeTimeout:              durationMS("HERMES_MCP_PROBE_TIMEOUT_MS", 60_000),
