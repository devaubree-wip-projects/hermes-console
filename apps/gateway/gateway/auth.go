...
var (
	serviceTimestampHeader    = gatewaycontracts.Spec.ServiceHeaders.Timestamp
	serviceSignatureHeader    = gatewaycontracts.Spec.ServiceHeaders.Signature
	serviceProfileHeader      = gatewaycontracts.Spec.ServiceHeaders.Profile
	serviceInstallationHeader = gatewaycontracts.Spec.ServiceHeaders.Installation
	serviceNonceHeader        = gatewaycontracts.Spec.ServiceHeaders.Nonce
)
...
func VerifyRuntimeTokenRequest(r *http.Request, expected string) bool {
	expected = strings.TrimSpace(expected)
	if expected == "" {
		return false
	}
	provided := strings.TrimSpace(r.Header.Get("X-Hermes-Session-Token"))
	if provided == "" {
